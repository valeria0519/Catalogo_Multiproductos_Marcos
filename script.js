...
function observeReveal() {
  document.querySelectorAll(
    ".reveal-up:not(.visible), .reveal-left:not(.visible), .cat-card-wrap:not(.visible), .card-wrap:not(.visible), .reveal-line:not(.visible)"
...
function cardHTML(p, delay = 0) {
  ...
  // Envuelto en card-wrap para animación de entrada escalonada
  return `
  <div class="card-wrap" style="--d:${delay}ms">
...
function renderGrid(id, lista) {
  ...
  if (!el) return;
  // Stagger delay: máx 60ms por item, tope en 480ms
  el.innerHTML = lista.length
    ? lista.map((p, i) => cardHTML(p, Math.min(i * 60, 480))).join("")
    : "";
...
// ══════════════════════════════════════════════
//  CATEGORÍAS HOME — cards grandes con stagger
// ══════════════════════════════════════════════
    ...
    <div class="det-grid">
      ...
      <div>
        ...
        <p style="font-size:11px;color:#888;margin-top:12px;text-align:center;line-height:1.5">
          Se enviará: <em>"Hola, quiero consultar por ${p.producto}${cuota?" de "+fmt(p.precio_cuota)+" en "+(p.cuotas||"?")+" cuotas":""}"</em>
        </p>
      </div>
    </div>`;
  irPagina("detalle");
...
function irPagina(nombre) {
  ...
  // Re-observar reveal al cambiar de página
...
// ← clic en categoría del home: va al catálogo filtrado
function irCatalogoFiltrado(cat) {
  paginaAnterior = "catalogo";
  irPagina("catalogo");
  // Esperar que el DOM esté visible antes de setear el filtro
  setTimeout(() => {
...
async function cargar() {
  ...
  try {
    ...
    ["grid-destacados","grid-catalogo"].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.innerHTML = `<p style="color:#e53935;padding:20px;text-align:center;grid-column:1/-1">
        ⚠️ No se pudieron cargar los productos.<br><small>${e.message}</small>
      </p>`;
    });
...
function renderTodo() {
  ...
  renderGrid("grid-destacados", dest.length ? dest : PRODS.slice(0, 4));
  aplicarFiltros();

  // Disparar reveal inicial con un pequeño delay para que el DOM esté listo
  setTimeout(observeReveal, 80);
}

cargar();

// Observar hero pills y textos que están en el DOM desde el inicio
